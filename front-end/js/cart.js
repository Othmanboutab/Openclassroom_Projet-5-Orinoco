...
`;
    ...
    for (i = 0; i < produitInLocalStorage.length; i++) {
        ...
        function multiple() {
            const totalPrice = produitInLocalStorage[i].price * produitInLocalStorage[i].quantite;
            return totalPrice;
        ...
        structurePanier += `
<div class="info">
<p> <strong>L'article : </strong>${produitInLocalStorage[i].name} &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; <strong>Lenses :</strong>${produitInLocalStorage[i].lenses} </p> 
...
for (let k = 0; k < btn_supprimer.length; k++) {
    ...
    btn_supprimer[k].addEventListener("click", (event) => {
        ...
        produitInLocalStorage = produitInLocalStorage.filter(e => e.id !== id_selection_supprimer);
...
function panierrempli() {
    let produitInLocalStorage = JSON.parse(localStorage.getItem("produit"));
    let cart = produitInLocalStorage.length;
    if (produitInLocalStorage !== null) {
        const select = document.querySelector(".nav")
        gabarithtml = `
        <a href="/front-end/index.html" class = "back"><i class="fas fa-arrow-left"></i></a>
        <a href="/front-end/index.html">orinoco</a>
        <a href="cart.html"> <i class="fas fa-cart-arrow-down"><p class="cercle"> <p class="chiffreCercle"> ${cart}</p></p></i></a>
